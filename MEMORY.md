...
每次巡检（心跳触发）完成时，必须做以下两件事：

1. **掌握规则变化** — 关注 `_ai/MEMORY.md`、`MEMORY.md`、`AGENTS.md` 等规则文件是否有更新。规则是活的，每次都得重新确认当前版本。
2. **了解其他 AI 分工变化** — 留意 Codex CLI、Claude Code 等协作 AI 的分工是否有调整。谁负责什么、怎么协作，不是一成不变的。

巡检结束后，把以上两点检查结果记录到 `memory/YYYY-MM-DD.md` 中。
